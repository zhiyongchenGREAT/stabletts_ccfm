...
class CFMDecoder(torch.nn.Module):
    ...
    def compute_loss(self, x1, mask, mu, c):
        ...
        fr = threshold_based_f_euler(r, segment_ends, xr, vr, self.boundary, x_at_segment_ends)
        
        loss_f = F.mse_loss(ft, fr, reduction="sum") / (torch.sum(mask) * ft.size(1))
        
        loss_v = self._masked_losses_v(vt, vr, self.boundary, segment_ends, t, mask)
        loss = loss_f + self.alpha * loss_v
        return loss, None
    
    def _masked_losses_v(self, vt, vr, threshold, segment_ends, t, mask):
      if threshold == 0:
        return 0
    
      less_than_threshold = t < threshold # [b, 1, 1]
...
def threshold_based_f_euler(t_expand, segment_ends, xt, vt, threshold, x_at_segment_ends):
    if threshold == 0:
