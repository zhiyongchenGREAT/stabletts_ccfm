...
from models.flow_matching import CFMDecoder, CFMDecoder_otmf
...
class StableTTS(nn.Module):
    def __init__(self, n_vocab, mel_channels, hidden_channels, filter_channels, n_heads, n_enc_layers, n_dec_layers, kernel_size, p_dropout, gin_channels, boundary):
        super().__init__()
        ...
        self.decoder = CFMDecoder(mel_channels, mel_channels, hidden_channels, mel_channels, filter_channels, n_heads, n_dec_layers, kernel_size, p_dropout, gin_channels, boundary)
    ...
    def synthesise(self, x, x_lengths, n_timesteps, temperature=1.0, y=None, length_scale=1.0, solver=None, cfg=1.0):
        ...
        ### why cfg set to 3 in inference ?
        if cfg == 1.0:
            decoder_outputs = self.decoder.forward_reflow_euler(mu_y, y_mask, n_timesteps, temperature, c, solver)
        else:
            cfg_kwargs = {'fake_speaker': self.fake_speaker, 'fake_content': self.fake_content, 'cfg_strength': cfg}
            decoder_outputs = self.decoder.forward_reflow_euler(mu_y, y_mask, n_timesteps, temperature, c, solver, cfg_kwargs)
...
# modified from https://github.com/shivammehta25/Matcha-TTS/blob/main/matcha/models/matcha_tts.py
class StableTTS_otfm(nn.Module):
    def __init__(self, n_vocab, mel_channels, hidden_channels, filter_channels, n_heads, n_enc_layers, n_dec_layers, kernel_size, p_dropout, gin_channels, boundary):
        super().__init__()
        ...
        self.ref_encoder = MelStyleEncoder(mel_channels, style_vector_dim=gin_channels, style_kernel_size=5, dropout=0.25)
        self.dp = DurationPredictor(hidden_channels, filter_channels, kernel_size, 0.5, gin_channels)
        self.decoder = CFMDecoder_otmf(mel_channels, mel_channels, hidden_channels, mel_channels, filter_channels, n_heads, n_dec_layers, kernel_size, p_dropout, gin_channels, boundary)
